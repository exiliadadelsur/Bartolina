...
def test_halo_properties_rad(bt, dmh):
    # halos radius are positive
    assert np.sum(dmh[0].radius < 0) == 0


def test_halo_properties_mass(bt, dmh):
...
def test_dark_matter_halos_radius(bt, dmh):
...
def test_dark_matter_halos_hmassive(bt, dmh):
